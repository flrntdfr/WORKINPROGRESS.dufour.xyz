...
description: |
    <p><b>Idea:</b> I wanted to explore a way to get functional code to an improbable problem using a Large Language Model (LLM).</p>
    <p><b>Strategy:</b> The strategy consists in prompting and helping the LLM write a standard Request For Comments (RFC) for the concept before implementing any code. The use case is in developing a non-standard title case converter in C.</p>
    <p><b>Prompt:</b> We will write a title case converter that implements human-centric design principles. The concept is named Human-centric Title Case (cased HcTc) and introduces casing as an attribute. The system improves readability via semantic compatibility. For example, it ensures capitalizing significant words while keeping functional (or generally low importance) words lowercase. It gracefully handles modern technical terms, acronyms, and proper nouns in general by preserving their original case (especially for brands). The concept is open first and exception-based.<br>
    ...
    <p><b>Result:</b> The LLM expressed the solution to the  Human-centric Title Case project in less than 2300 words and implemented C code. It pdated the Nix envirionment to get access to the compiler and compiled the code. It finally got a demo to run in the browser using WebAssembly and less that  2 pages (131072 bytes) of memory.</p>
href:
